...
$(function(){
	...
	html += '                    </div>';
	html += '                    <div class="float-left margin-small-left">';
	html += '                        <div class="txt radius-small bg text-gray text-large add" attr="hour"><i class="icon iconfont text-diysize4 camnpr-rotate-180">&#xe698;</i></div>';
	html += '                        <div class="hour padding-small-top padding-small-bottom">10时</div>';
	html += '                        <div class="txt radius-small bg text-gray text-large minus" attr="hour"><i class="icon iconfont text-diysize4">&#xe698;</i></div>';
	html += '                    </div>';
	html += '                    <div class="float-left margin-small-left">';
	html += '                        <div class="txt radius-small bg text-gray text-large add" attr="min"><i class="icon iconfont text-diysize4 camnpr-rotate-180">&#xe698;</i></div>';
	html += '                        <div class="min padding-small-top padding-small-bottom">10分</div>';
	html += '                        <div class="txt radius-small bg text-gray text-large minus" attr="min"><i class="icon iconfont text-diysize4">&#xe698;</i></div>';
	html += '                    </div>';
	html += '                </div>';
	html += '            </div>';
	html += '        </div>';
    ...
    function dateInit(obj){
        ...
        dateArr=curDate[0].split('-');
        curYear=dateArr[0];
        curMonth=dateArr[1];
        curDay=dateArr[2];
		var timeArr = new Array;
        if(typeof(curDate[1])!="undefined")timeArr=curDate[1].split(':');
        curHour=typeof(timeArr[0])=="undefined"?myDate.getHours():timeArr[0];
        curMin=typeof(timeArr[1])=="undefined"?myDate.getMinutes():timeArr[1];
        curSec=typeof(timeArr[2])=="undefined"?myDate.getSeconds():timeArr[2];
        ...
        if($('.chose-date-dialog').find('.hour')){
            $('.chose-date-dialog').find('.hour').text(curHour+'时');
        }
    ...
    $('.chose-date-dialog').find('.sure').click(function(){
        ...
        if(!bool){
            ...
            if(flag=='4'){
                $('.btn-dialog').text(year+'年'+month+'月'+day+'日'+hour+'时'+min+'分');
                $('.date-input').val(year+'-'+month+'-'+day+' '+hour+':'+min);
            }
			else{
