...
function datebind(callback){
	...
	html += '    <div class="main">';
	html += '        <div class="padding-little-top padding-little-bottom">';
	html += '            <div class="box">';
	html += '                <div class="clearfix">';
	html += '                    <div class="txt radius-small bg text-gray float-left text-large add-year">+</div>';
	html += '                    <div class="txt radius-small bg text-gray float-left margin-small-left text-large add-month">+</div>';
	html += '                    <div class="txt radius-small bg text-gray float-left margin-small-left text-large add-day">+</div>';
	html += '                </div>';
	...
	html += '                    <div class="float-left day">10日</div>';
	html += '                </div>';
	html += '                <div class="clearfix">';
	html += '                    <div class="txt radius-small bg text-gray float-left text-large minus-year">-</div>';
	html += '                    <div class="txt radius-small bg text-gray float-left margin-small-left text-large minus-month">-</div>';
	html += '                    <div class="txt radius-small bg text-gray float-left margin-small-left text-large minus-day">-</div>';
	html += '                </div>';
	html += '            </div>';
	html += '        </div>';
	html += '    </div>';
	...
	$('.date-chose').find('a').click(function(){
		...
		var txtM=txt.substring(5,7);
		var txtD=txt.substring(8,txt.length);
		$('.chose-date-dialog').find('.year').empty().text(txtY);
		$('.chose-date-dialog').find('.month').empty().text(txtM);
		$('.chose-date-dialog').find('.month').empty().text(txtD);
	...
	$('.chose-date-dialog').find('.add-day').click(function(){
		var txtM=$(this).parents('.chose-date-dialog').find('.day').text();
		var txtMN=parseInt(txtM);
		var txtMS="00"+(txtMN+1);
		txtMS = txtMS.substring(txtMS.length-2,txtMS.length) +'日';
		if((parseInt(txtMN)-1)>=31){
			txtMS='01日';
		}
		var headStr=$(this).parents('.chose-date-dialog').find('.head').text();
		var headSY=headStr.substring(0,5);
