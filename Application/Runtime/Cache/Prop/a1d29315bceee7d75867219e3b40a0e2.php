...
<section class="margin-top">
    <div class="form-group select-group padding-small bg-white  border-bottom border-top margin-top">
        ...
        <div class="field">
            <select name="house" class="input">
                <?php if(is_array($house_list)): $i = 0; $__LIST__ = $house_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i;?><option value="<?php echo ($option['room_number']); ?>" user_house_id="<?php echo ($option['user_house_id']); ?>" project_id="<?php echo ($option['project_id']); ?>" project_name="<?php echo ($option['project_name']); ?>" building="<?php echo ($option['building']); ?>" room_number="<?php echo ($option['room_number']); ?>"><?php echo ($option['project_name']); echo ($option['building']); ?>(栋)<?php echo ($option['room_number']); ?>号</option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </div>
...
$(function(){
	$("button[name='submit']").click(function(){
		...
		var project_id = $("select[name='house'] option:selected").attr("project_id");
